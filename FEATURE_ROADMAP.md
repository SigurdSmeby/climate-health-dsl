...
| 4 | Implemented | `from_csv` reads any CHAP-format CSV (`file`, `column`, `source_location`, `start_period` params); insufficient data is a hard error, never wrapped or extrapolated. Multi-location: with no `source_location`, each output location auto-matches the CSV rows of the same name (so each gets its own real climate); an explicit `source_location` feeds one source to all. |
