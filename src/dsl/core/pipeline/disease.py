...
def _standardize(series: np.ndarray) -> np.ndarray:
    ...
    if std == 0:
        # A constant series carries no signal → zeros, but keep the NaN
        # positions so those disease rows stay blanked, not fabricated.
...
def _draw_counts(
    ...
    Var = rate + rate²/k — smaller k means spikier counts.
    """
...
def build_disease_cases(
    ...
    Returns a float array of length ``n_periods`` holding integer counts,
    with NaN in the lag warm-up and wherever ``missing_rate`` struck.
    """
    ppy = periods_per_year(period)
    t = np.arange(n_periods)

    # 1. Seasonal baseline, so disease has its own seasonality even with no
    #    drivers attached.
    eta = np.sin(2 * np.pi * (t % ppy) / ppy)

    # 2. Add each lagged, transformed, standardized, weighted driver. A
    #    weight-0 dependency is skipped so its NaNs don't blank rows it
    #    doesn't influence.
    ...
    # 3. Optional autoregressive component: a random walk gives the signal
    ...
    # 4. eta → incidence rate. The logit shift maps eta = 0 (a typical
    #    period) to median_rate; the sigmoid keeps every rate below
    ...
    # 5. Draw counts and cap at the population.
