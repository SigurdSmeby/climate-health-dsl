...
def test_from_csv_unsorted_periods_are_sorted(tmp_path, rng):
    ...
    csv = write_csv(
        tmp_path / "unsorted.csv", ["2010-03", "2010-01", "2010-02", "2010-04"],
...
def test_from_csv_unsorted_without_start_period(tmp_path, rng):
    csv = write_csv(
        tmp_path / "unsorted2.csv", ["2010-03", "2010-01", "2010-02"],
...
def test_from_csv_duplicate_periods_rejected(tmp_path, rng):
    # A repeated period silently shifts alignment — reject it.
    csv = write_csv(
        tmp_path / "dup.csv", ["2010-01", "2010-01", "2010-02"],
...
def test_from_csv_requires_time_period_for_start(tmp_path, rng):
    # No time_period column means start_period can't be honored.
...
def test_from_csv_empty_file(tmp_path, rng):
    # An empty file must give a clear from_csv error.
...
def test_from_csv_header_only(tmp_path, rng):
    # A header with no data rows must give a clear error, not IndexError.
    csv = tmp_path / "ho.csv"
...
def test_from_csv_non_numeric_clear_error(tmp_path, rng):
    # Text in the numeric column should give a clear from_csv message.
    csv = write_csv(
        tmp_path / "text.csv", ["2010-01", "2010-02", "2010-03"],
        rainfall=["1", "heavy", "3"],
    )
    gen = FromCsvGenerator(file=str(csv), column="rainfall")
...
def test_from_csv_gap_in_periods_rejected(tmp_path, rng):
    # A missing period in the middle must be rejected, not silently
    # relabelled (Feb absent -> Mar's value written as Feb).
    csv = write_csv(
        tmp_path / "gaps.csv", ["2010-01", "2010-03", "2010-04"],
        rainfall=[10.0, 30.0, 40.0],
    )
    gen = FromCsvGenerator(file=str(csv), column="rainfall")
...
def test_from_csv_consecutive_periods_ok(tmp_path, rng):
    csv = write_csv(
        tmp_path / "ok.csv", ["2010-01", "2010-02", "2010-03"],
...
def test_from_csv_impossible_monthly_label_rejected(tmp_path, rng, label):
    # Shape-valid but impossible calendar labels must be rejected.
...
def test_from_csv_infinite_value_rejected(tmp_path, rng):
    # Inf is numeric but not a valid covariate value.
    csv = write_csv(
        tmp_path / "inf.csv", ["2010-01", "2010-02", "2010-03"],
